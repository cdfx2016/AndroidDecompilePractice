...
public final class Cookie {
    ...
    private static boolean domainMatch(HttpUrl url, String domain) {
        ...
        if (urlHost.endsWith(domain) && urlHost.charAt((urlHost.length() - domain.length()) - 1) == FilenameUtils.EXTENSION_SEPARATOR && !Util.verifyAsIpAddress(urlHost)) {
            return true;
        }
        return false;
    }
    ...
    private static boolean pathMatch(HttpUrl url, String path) {
        ...
        if (urlPath.startsWith(path) && (path.endsWith("/") || urlPath.charAt(path.length()) == IOUtils.DIR_SEPARATOR_UNIX)) {
            return true;
        }
        return false;
    }
    ...
    private static String parseDomain(String s) {
        if (s.endsWith(FileUtil.FILE_EXTENSION_SEPARATOR)) {
            throw new IllegalArgumentException();
        }
        if (s.startsWith(FileUtil.FILE_EXTENSION_SEPARATOR)) {
            s = s.substring(1);
        }
