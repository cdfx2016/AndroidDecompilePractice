...
public final class OkHostnameVerifier implements HostnameVerifier {
    ...
    private boolean verifyHostname(String hostname, String pattern) {
        if (hostname == null || hostname.length() == 0 || hostname.startsWith(FileUtil.FILE_EXTENSION_SEPARATOR) || hostname.endsWith("..") || pattern == null || pattern.length() == 0 || pattern.startsWith(FileUtil.FILE_EXTENSION_SEPARATOR) || pattern.endsWith("..")) {
            return false;
        }
        if (!hostname.endsWith(FileUtil.FILE_EXTENSION_SEPARATOR)) {
            hostname = hostname + FilenameUtils.EXTENSION_SEPARATOR;
        }
        if (!pattern.endsWith(FileUtil.FILE_EXTENSION_SEPARATOR)) {
