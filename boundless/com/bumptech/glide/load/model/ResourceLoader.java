...
public class ResourceLoader<T> implements ModelLoader<Integer, T> {
    ...
    public DataFetcher<T> getResourceFetcher(Integer model, int width, int height) {
        Uri uri = null;
        try {
            uri = Uri.parse("android.resource://" + this.resources.getResourcePackageName(model.intValue()) + IOUtils.DIR_SEPARATOR_UNIX + this.resources.getResourceTypeName(model.intValue()) + IOUtils.DIR_SEPARATOR_UNIX + this.resources.getResourceEntryName(model.intValue()));
