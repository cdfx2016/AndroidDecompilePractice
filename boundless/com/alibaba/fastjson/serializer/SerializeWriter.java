...
public final class SerializeWriter extends Writer {
        ...
        if (isEnabled(SerializerFeature.BrowserSecure)) {
            ...
            for (i = lastSpecialIndex; i >= start; i--) {
                ...
                if ((ch2 < '0' || ch2 > '9') && ((ch2 < 'a' || ch2 > 'z') && !((ch2 >= 'A' && ch2 <= 'Z') || ch2 == ',' || ch2 == '.' || ch2 == '_'))) {
                    ...
                    this.buf[i] = cn.finalteam.toolsfinal.io.IOUtils.DIR_SEPARATOR_WINDOWS;
                    this.buf[i + 1] = 'u';
            ...
            if (specialCount > 0) {
                ...
                if (specialCount == 1) {
                    ...
                    if (lastSpecial == ' ') {
                        ...
                        this.buf[lastSpecialIndex] = cn.finalteam.toolsfinal.io.IOUtils.DIR_SEPARATOR_WINDOWS;
                        lastSpecialIndex++;
                        ...
                        if (ch2 >= IOUtils.specicalFlags_doubleQuotes.length || IOUtils.specicalFlags_doubleQuotes[ch2] != (byte) 4) {
                            ...
                            i2 = lastSpecialIndex;
                            i3 = i2 + 1;
                            this.buf[i2] = cn.finalteam.toolsfinal.io.IOUtils.DIR_SEPARATOR_WINDOWS;
                            i2 = i3 + 1;
                            this.buf[i3] = 'u';
                            i3 = i2 + 1;
        ...
        for (int i = 0; i < list_size; i++) {
            ...
            if (text != null) {
                ...
                for (j = 0; j < len; j++) {
                    ...
                    hasSpecial = ch < ' ' || ch > '~' || ch == '\"' || ch == cn.finalteam.toolsfinal.io.IOUtils.DIR_SEPARATOR_WINDOWS;
                    if (hasSpecial) {
                        break;
                    }
                }
            ...
            if (specialCount == 1) {
                ...
                if (lastSpecial == ' ') {
                    ...
                    if (ch >= IOUtils.specicalFlags_doubleQuotes.length || IOUtils.specicalFlags_doubleQuotes[ch] != (byte) 4) {
                        ...
                        i4 = lastSpecialIndex;
                        i5 = i4 + 1;
                        this.buf[i4] = cn.finalteam.toolsfinal.io.IOUtils.DIR_SEPARATOR_WINDOWS;
                        i4 = i5 + 1;
                        this.buf[i5] = 'u';
                        i5 = i4 + 1;
    ...
    static boolean isSpecial(char ch, int features) {
        ...
        if (ch == cn.finalteam.toolsfinal.io.IOUtils.DIR_SEPARATOR_UNIX) {
            if ((SerializerFeature.WriteSlashAsSpecial.mask & features) == 0) {
                z = false;
            }
            return z;
        } else if (ch > '#' && ch != cn.finalteam.toolsfinal.io.IOUtils.DIR_SEPARATOR_WINDOWS) {
            return false;
        } else {
            if (ch <= '\u001f' || ch == cn.finalteam.toolsfinal.io.IOUtils.DIR_SEPARATOR_WINDOWS || ch == '\"') {
                return true;
            }
            return false;
        }
    ...
    protected void writeStringWithSingleQuote(String text) {
        ...
        if (newcount > this.buf.length) {
            if (this.writer != null) {
                ...
                for (i = 0; i < text.length(); i++) {
                    int ch = text.charAt(i);
                    if (ch <= '\r' || ch == cn.finalteam.toolsfinal.io.IOUtils.DIR_SEPARATOR_WINDOWS || ch == '\'' || (ch == cn.finalteam.toolsfinal.io.IOUtils.DIR_SEPARATOR_UNIX && isEnabled(SerializerFeature.WriteSlashAsSpecial))) {
                        write(92);
        ...
        for (i = start; i < end; i++) {
            char ch2 = this.buf[i];
            if (ch2 <= '\r' || ch2 == cn.finalteam.toolsfinal.io.IOUtils.DIR_SEPARATOR_WINDOWS || ch2 == '\'' || (ch2 == cn.finalteam.toolsfinal.io.IOUtils.DIR_SEPARATOR_UNIX && isEnabled(SerializerFeature.WriteSlashAsSpecial))) {
                specialCount++;
                lastSpecialIndex = i;
                lastSpecial = ch2;
            }
