...
public class JSONPath implements JSONAware {
    ...
    private static ConcurrentMap<String, JSONPath> pathCache = new ConcurrentHashMap(128, AdaptiveVideoTrackSelection.DEFAULT_BANDWIDTH_FRACTION, 1);
    ...
    static class JSONPathParser {
        ...
        Segement readSegement() {
            ...
            while (!isEOF()) {
                ...
                if (this.ch == '$') {
                    next();
                } else if (this.ch == FilenameUtils.EXTENSION_SEPARATOR || this.ch == IOUtils.DIR_SEPARATOR_UNIX) {
                    int c0 = this.ch;
                    boolean deep = false;
                    next();
                    if (c0 == 46 && this.ch == FilenameUtils.EXTENSION_SEPARATOR) {
                        next();
                        deep = true;
                    }
                    if (this.ch == '*') {
        ...
        Segement parseArrayAccess(boolean acceptBracket) {
            ...
            if (predicateFlag || com.alibaba.fastjson.util.IOUtils.firstIdentifier(this.ch)) {
                String propertyName = readName();
                skipWhitespace();
            ...
            while (this.ch != ']' && this.ch != IOUtils.DIR_SEPARATOR_UNIX && !isEOF() && (this.ch != FilenameUtils.EXTENSION_SEPARATOR || predicateFlag || predicateFlag)) {
                if (this.ch == IOUtils.DIR_SEPARATOR_WINDOWS) {
                    next();
                }
                next();
            }
            ...
            } else if (this.ch == IOUtils.DIR_SEPARATOR_UNIX || this.ch == FilenameUtils.EXTENSION_SEPARATOR) {
                end = this.pos - 1;
            } else {
                end = this.pos;
            }
        ...
        String readName() {
            skipWhitespace();
            if (this.ch == IOUtils.DIR_SEPARATOR_WINDOWS || com.alibaba.fastjson.util.IOUtils.firstIdentifier(this.ch)) {
                StringBuilder buf = new StringBuilder();
                while (!isEOF()) {
                    if (this.ch != IOUtils.DIR_SEPARATOR_WINDOWS) {
                        if (!com.alibaba.fastjson.util.IOUtils.isIdent(this.ch)) {
                            break;
                        }
                        buf.append(this.ch);
                        next();
                ...
                if (isEOF() && com.alibaba.fastjson.util.IOUtils.isIdent(this.ch)) {
                    buf.append(this.ch);
                }
                return buf.toString();
            }
        ...
        Segement buildArraySegement(String indexText) {
            ...
            if (indexText.length() <= 2 || firstChar != '\'' || lastChar != '\'') {
                ...
                int[] indexes;
                if (commaIndex != -1) {
                    indexesText = indexText.split(Constants.ACCEPT_TIME_SEPARATOR_SP);
    ...
    protected Object getPropertyValue(Object currentObject, String propertyName, boolean strictMode) {
        ...
        if (beanSerializer != null) {
            ...
            if (currentObject instanceof Enum) {
                ...
                if (Preferences.sbry.equals(propertyName)) {
                    return e2.name();
                }
