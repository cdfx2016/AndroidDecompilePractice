...
public abstract class JSONLexerBase implements JSONLexer, Closeable {
    ...
    public final void nextToken() {
        ...
        while (true) {
            this.pos = this.bp;
            if (this.ch == IOUtils.DIR_SEPARATOR_UNIX) {
                skipComment();
            } else if (this.ch == '\"') {
                scanString();
                return;
    ...
    public final void scanString() {
        ...
        while (true) {
            ...
            } else if (ch == IOUtils.DIR_SEPARATOR_WINDOWS) {
                if (!this.hasSpecial) {
        ...
        if (charArrayCompare(fieldName)) {
            ...
            if (stringVal.indexOf(92) != -1) {
                while (true) {
                    int slashCount = 0;
                    int i = endIndex - 1;
                    while (i >= 0 && charAt(i) == IOUtils.DIR_SEPARATOR_WINDOWS) {
                        slashCount++;
                        i--;
                    }
                    if (slashCount % 2 == 0) {
    ...
    public String scanFieldSymbol(char[] fieldName, SymbolTable symbolTable) {
        ...
        if (charArrayCompare(fieldName)) {
            ...
            while (true) {
                ...
                hash = (hash * 31) + chLocal;
                if (chLocal == IOUtils.DIR_SEPARATOR_WINDOWS) {
                    this.matchStat = -1;
                    return null;
                }
                offset = offset2;
    ...
    public int scanFieldInt(char[] fieldName) {
        ...
        if (charArrayCompare(fieldName)) {
            ...
            if (chLocal == FilenameUtils.EXTENSION_SEPARATOR) {
                this.matchStat = -1;
                return 0;
    ...
    public static String readString(char[] chars, int chars_len) {
        ...
        while (i < chars_len) {
            int len2;
            char ch = chars[i];
            if (ch != IOUtils.DIR_SEPARATOR_WINDOWS) {
                len2 = len + 1;
                sbuf[len] = ch;
            } else {
                i++;
                switch (chars[i]) {
                    ...
                    case '/':
                        len2 = len + 1;
                        sbuf[len] = IOUtils.DIR_SEPARATOR_UNIX;
                        break;
                    case '0':
                        len2 = len + 1;
                        sbuf[len] = '\u0000';
                    ...
                    case '\\':
                        len2 = len + 1;
                        sbuf[len] = IOUtils.DIR_SEPARATOR_WINDOWS;
                        break;
                    case 'b':
                        len2 = len + 1;
                        sbuf[len] = '\b';
    ...
    public final void skipWhitespace() {
        while (this.ch <= IOUtils.DIR_SEPARATOR_UNIX) {
            if (this.ch == ' ' || this.ch == '\r' || this.ch == '\n' || this.ch == '\t' || this.ch == '\f' || this.ch == '\b') {
                next();
            } else if (this.ch == IOUtils.DIR_SEPARATOR_UNIX) {
                skipComment();
            } else {
                return;
            }
    ...
    private void scanStringSingleQuote() {
        ...
        while (true) {
            ...
            if (chLocal == '\'') {
                ...
                putChar(JSONLexer.EOI);
            } else if (chLocal == IOUtils.DIR_SEPARATOR_WINDOWS) {
                if (!this.hasSpecial) {
                    this.hasSpecial = true;
                    if (this.sp > this.sbuf.length) {
