...
public final class UriUtil {
    ...
    public static String resolve(String baseUri, String referenceUri) {
        ...
        if (refIndices[1] != 0) {
            ...
            uri.append(baseUri, 0, baseIndices[1]).append(IOUtils.DIR_SEPARATOR_UNIX).append(referenceUri);
    ...
    private static String removeDotSegments(StringBuilder uri, int offset, int limit) {
        ...
        while (i <= limit) {
            ...
            if (i == segmentStart + 1 && uri.charAt(segmentStart) == FilenameUtils.EXTENSION_SEPARATOR) {
                uri.delete(segmentStart, nextSegmentStart);
                limit -= nextSegmentStart - segmentStart;
                i = segmentStart;
            } else if (i == segmentStart + 2 && uri.charAt(segmentStart) == FilenameUtils.EXTENSION_SEPARATOR && uri.charAt(segmentStart + 1) == FilenameUtils.EXTENSION_SEPARATOR) {
                int removeFrom;
    ...
    private static int[] getUriIndices(String uriString) {
        ...
        if (TextUtils.isEmpty(uriString)) {
            ...
            if (schemeIndex + 2 < queryIndex && uriString.charAt(schemeIndex + 1) == IOUtils.DIR_SEPARATOR_UNIX && uriString.charAt(schemeIndex + 2) == IOUtils.DIR_SEPARATOR_UNIX) {
                hasAuthority = true;
            } else {
                hasAuthority = false;
            }
