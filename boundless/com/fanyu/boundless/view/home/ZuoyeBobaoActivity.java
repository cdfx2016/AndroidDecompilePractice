...
public class ZuoyeBobaoActivity extends BaseActivity<ZuoyeBoBaoPresenter> implements IZuoyeBoBaoView, PullLoadMoreListener {
    public static ZuoyeBobaoActivity zuoyeBobaoActivity;
    private OnClickListener _poptextClickListener = new OnClickListener() {
        public void onClick(View v) {
            switch (v.getId()) {
                case R.id.selectclass_pop_all:
                    ZuoyeBobaoActivity.this.mSpinerPopWindow.dismiss();
                    return;
                default:
                    return;
            }
    ...
    private OnItemClickListener itemClickListener = new OnItemClickListener() {
        public void onItemClick(AdapterView<?> adapterView, View view, int position, long id) {
            ZuoyeBobaoActivity.this.className = (String) ZuoyeBobaoActivity.this.list.get(position);
            if (StringUtils.isEmpty(ZuoyeBobaoActivity.this.className)) {
                ZuoyeBobaoActivity.this.classname.setText(ZuoyeBobaoActivity.this.className);
            }
            ZuoyeBobaoActivity.this.mSpinerPopWindow.dismiss();
            if (ZuoyeBobaoActivity.this.className.contains("全部班级")) {
                ZuoyeBobaoActivity.this.posthomeworkentityApi.setClassid("");
                ((ZuoyeBoBaoPresenter) ZuoyeBobaoActivity.this.mPresenter).startPost(ZuoyeBobaoActivity.this, ZuoyeBobaoActivity.this.posthomeworkentityApi, 0);
                return;
            }
            ZuoyeBobaoActivity.this.posthomeworkentityApi.setClassid(((schoolclassentity) ZuoyeBobaoActivity.this.schoolclasslist.get(position)).getId());
            ((ZuoyeBoBaoPresenter) ZuoyeBobaoActivity.this.mPresenter).startPost(ZuoyeBobaoActivity.this, ZuoyeBobaoActivity.this.posthomeworkentityApi, 0);
        }
    };
    ...
    private int page = 1;
    private Posthomeworkentity posthomeworkentity = new Posthomeworkentity();
    ...
    protected void init() {
        ...
        this.posthomeworkentityApi.setHwtype("0");
    ...
    public void getIZuoyeBoBao(List<Posthomeworkentity> result, int state) {
        if (state == 0) {
            ...
            this.zuoYeAdapter = new ZuoYeBoBaoNewAdapter(this.mContext, R.layout.adapter_zuoyebobaolist, this.datalist, this.userid, "0");
    ...
    public void helloEventBus(UpdateZuoyeEvent updateZuoyeEvent) {
        onRefresh();
    }

    @Subscribe(threadMode = ThreadMode.MainThread)
    public void helloEventBus(UpdateZuoYeAdapterEvent updateZuoYeAdapterEvent) {
        if (updateZuoYeAdapterEvent.getType().equals("0")) {
            onRefresh();
        }
    }
    ...
    public void helloEventBus(DeleteZuoYeEvent deleteZuoYeEvent) {
        if (deleteZuoYeEvent.getType().equals("0")) {
            onRefresh();
        }
    }
