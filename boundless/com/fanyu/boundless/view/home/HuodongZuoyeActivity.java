...
public class HuodongZuoyeActivity extends BaseActivity<ZuoyeBoBaoPresenter> implements IZuoyeBoBaoView, PullLoadMoreListener {
    public static HuodongZuoyeActivity zuoyeBobaoActivity;
    private OnClickListener _poptextClickListener = new OnClickListener() {
        public void onClick(View v) {
            switch (v.getId()) {
                case R.id.selectclass_pop_all:
                    HuodongZuoyeActivity.this.mSpinerPopWindow.dismiss();
                    return;
                default:
                    return;
            }
    ...
    private OnItemClickListener itemClickListener = new OnItemClickListener() {
        public void onItemClick(AdapterView<?> adapterView, View view, int position, long id) {
            HuodongZuoyeActivity.this.className = (String) HuodongZuoyeActivity.this.list.get(position);
            if (StringUtils.isEmpty(HuodongZuoyeActivity.this.className)) {
                HuodongZuoyeActivity.this.classname.setText(HuodongZuoyeActivity.this.className);
            }
            HuodongZuoyeActivity.this.mSpinerPopWindow.dismiss();
            if (HuodongZuoyeActivity.this.className.contains("全部班级")) {
                HuodongZuoyeActivity.this.posthomeworkentityApi.setClassid("");
                ((ZuoyeBoBaoPresenter) HuodongZuoyeActivity.this.mPresenter).startPost(HuodongZuoyeActivity.this, HuodongZuoyeActivity.this.posthomeworkentityApi, 0);
                return;
            }
            HuodongZuoyeActivity.this.posthomeworkentityApi.setClassid(((schoolclassentity) HuodongZuoyeActivity.this.schoolclasslist.get(position)).getId());
            ((ZuoyeBoBaoPresenter) HuodongZuoyeActivity.this.mPresenter).startPost(HuodongZuoyeActivity.this, HuodongZuoyeActivity.this.posthomeworkentityApi, 0);
        }
    };
    ...
    protected void init() {
        ...
        this.messageTitle.setText("活动作业");
        ...
        this.posthomeworkentityApi.setHwtype("1");
    ...
    public void getIZuoyeBoBao(List<Posthomeworkentity> result, int state) {
        if (state == 0) {
            ...
            this.zuoYeAdapter = new ZuoYeBoBaoNewAdapter(this.mContext, R.layout.adapter_zuoyebobaolist, this.datalist, this.userid, "1");
    ...
    public void helloEventBus(UpdateHuoDongEvent updateHuoDongEvent) {
        onRefresh();
    }

    @Subscribe(threadMode = ThreadMode.MainThread)
    public void helloEventBus(UpdateZuoYeAdapterEvent updateZuoYeAdapterEvent) {
        if (updateZuoYeAdapterEvent.getType().equals("1")) {
            onRefresh();
        }
    }
    ...
    public void helloEventBus(DeleteZuoYeEvent deleteZuoYeEvent) {
        if (deleteZuoYeEvent.getType().equals("1")) {
            onRefresh();
        }
    }
