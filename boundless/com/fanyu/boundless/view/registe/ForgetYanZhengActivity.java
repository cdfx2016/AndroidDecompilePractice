...
public class ForgetYanZhengActivity extends BaseActivity<YanZhengPresenter> implements IZhuCeYanZhengView {
    Handler handler = new Handler() {
        public void handleMessage(Message msg) {
            ...
            if (result != -1) {
                Toast.makeText(ForgetYanZhengActivity.this.getApplicationContext(), "验证码输入错误，请重新输入！", 0).show();
            } else if (event == 3) {
                Toast.makeText(ForgetYanZhengActivity.this.getApplicationContext(), "提交验证码成功", 0).show();
                Intent intent = new Intent(ForgetYanZhengActivity.this, ResetPassWordActivity.class);
                intent.putExtra("telephone", ForgetYanZhengActivity.this.phoneNums);
                ForgetYanZhengActivity.this.startActivity(intent);
                ForgetYanZhengActivity.this.finish();
            } else if (event == 2) {
                Toast.makeText(ForgetYanZhengActivity.this.getApplicationContext(), "验证码已经发送", 0).show();
            } else {
    ...
    private CountDownTimer timer = new CountDownTimer(60000, 1000) {
        public void onTick(long millisUntilFinished) {
            ForgetYanZhengActivity.this.loginRequestCodeBtn.setText((millisUntilFinished / 1000) + "秒后可重发");
        }

        public void onFinish() {
            ForgetYanZhengActivity.this.loginRequestCodeBtn.setClickable(true);
            ForgetYanZhengActivity.this.loginRequestCodeBtn.setText("获取验证码");
        }
    };
    ...
    protected void init() {
        SMSSDK.registerEventHandler(new EventHandler() {
            public void afterEvent(int event, int result, Object data) {
                ...
                msg.arg2 = result;
                msg.obj = data;
                ForgetYanZhengActivity.this.handler.sendMessage(msg);
            }
        });
    }
    ...
    public void getLoginYanZheng(Login login) {
        if (login.getStatus() == 2) {
            ...
            Toast.makeText(this, "您还未注册，请您先注册！", 0).show();
        }
    }
}
